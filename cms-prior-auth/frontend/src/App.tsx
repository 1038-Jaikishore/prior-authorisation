...
export default function App() {
  ...
  // Post-Volume 8 Intake Document States
  const [showIntakeWorkflow, setShowIntakeWorkflow] = useState<boolean>(false);
  const [uploadFile, setUploadFile] = useState<File | null>(null);
  const [uploading, setUploading] = useState<boolean>(false);
  const [uploadedDocMetadata, setUploadedDocMetadata] = useState<any | null>(null);
  const [extractedFacts, setExtractedFacts] = useState<any | null>(null);
  const [confirming, setConfirming] = useState<boolean>(false);
  const [intakeHcpcsOverride, setIntakeHcpcsOverride] = useState<string>('');
  const [intakeStateOverride, setIntakeStateOverride] = useState<string>('CO');
  const [intakeDosOverride, setIntakeDosOverride] = useState<string>('');
  const [extractionError, setExtractionError] = useState<string | null>(null);
  const [activeProvenanceHighlight, setActiveProvenanceHighlight] = useState<any | null>(null);
  ...
  const handleDocumentUploadAndExtract = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      ...
      setUploadedDocMetadata(uploadMeta);

      // 2. Extract
      const extractRes = await fetch(`http://localhost:8000/api/documents/${uploadMeta.document_id}/extract`, {
        method: 'POST',
      });
      if (!extractRes.ok) {
        const detail = await extractRes.json();
        throw new Error(detail.detail || 'Extraction failed.');
      }
      const facts = await extractRes.json();
      setExtractedFacts(facts);

      // Auto-set overrides if present in extraction
      setIntakeHcpcsOverride(facts.requested_service?.code || '');
      setIntakeStateOverride(facts.geography?.state || 'CO');
      setIntakeDosOverride(facts.requested_service?.date_of_service || '');
    } catch (err: any) {
  ...
  const handleSaveDraftCorrections = async () => {
    if (!uploadedDocMetadata || !extractedFacts) return;
    setConfirming(true);
    setExtractionError(null);

    try {
      const res = await fetch(`http://localhost:8000/api/documents/${uploadedDocMetadata.document_id}/extraction?reviewer_id=${reviewerId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(extractedFacts),
      });
      if (!res.ok) {
        const detail = await res.json();
        throw new Error(detail.detail || 'Failed to save draft edits.');
      }
      const updated = await res.json();
      setExtractedFacts(updated);
      alert('✓ Draft extraction corrections saved successfully.');
    } catch (err: any) {
      setExtractionError(err.message);
    } finally {
      setConfirming(false);
  ...
  const handleConfirmExtraction = async () => {
    ...
    try {
      ...
      const saveRes = await fetch(`http://localhost:8000/api/documents/${uploadedDocMetadata.document_id}/extraction?reviewer_id=${reviewerId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(extractedFacts),
      });
      if (!saveRes.ok) {
        const detail = await saveRes.json();
        throw new Error(detail.detail || 'Failed to save edits before confirmation.');
      }
      const updatedFacts = await saveRes.json();
      setExtractedFacts(updatedFacts);

      // Confirm
      const confirmRes = await fetch(`http://localhost:8000/api/documents/${uploadedDocMetadata.document_id}/confirm?reviewer_id=${reviewerId}`, {
        method: 'POST',
      });
      if (!confirmRes.ok) {
        const detail = await confirmRes.json();
        throw new Error(detail.detail || 'Confirmation failed.');
      }

      setUploadedDocMetadata((prev: any) => ({ ...prev, upload_status: 'CONFIRMED' }));
      setExtractedFacts((prev: any) => ({ ...prev, status: 'CONFIRMED' }));
      alert('✓ Extraction facts confirmed and locked.');
    } catch (err: any) {
      setExtractionError(err.message);
    } finally {
      setConfirming(false);
  ...
  const handleGeneratePriorAuth = async () => {
    ...
    try {
      ...
      if (extractedFacts.status !== 'CONFIRMED') {
        // 1. Save corrections
        const saveRes = await fetch(`http://localhost:8000/api/documents/${uploadedDocMetadata.document_id}/extraction?reviewer_id=${reviewerId}`, {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
        ...
        if (!saveRes.ok) {
          const detail = await saveRes.json();
          throw new Error(detail.detail || 'Failed to save edits before evaluation.');
        }
        const updatedFacts = await saveRes.json();
        setExtractedFacts(updatedFacts);
        ...
        if (!confirmRes.ok) {
          const detail = await confirmRes.json();
          throw new Error(detail.detail || 'Failed to confirm extraction facts.');
        }
        setUploadedDocMetadata((prev: any) => ({ ...prev, upload_status: 'CONFIRMED' }));
        setExtractedFacts((prev: any) => ({ ...prev, status: 'CONFIRMED' }));
      }

      // 3. Now execute document evaluation orchestrator
      const params = new URLSearchParams({
        reviewer_id: reviewerId,
      });
      if (intakeHcpcsOverride) params.append('hcpcs_override', intakeHcpcsOverride);
      if (intakeStateOverride) params.append('state_override', intakeStateOverride);
      if (intakeDosOverride) params.append('dos_override', intakeDosOverride);

      const res = await fetch(`http://localhost:8000/api/documents/${uploadedDocMetadata.document_id}/evaluate?${params.toString()}`, {
        method: 'POST',
      });
      if (!res.ok) {
        const detail = await res.json();
        throw new Error(detail.detail || 'Failed to run CMS Prior-Auth Evaluation.');
      }
      const data = await res.json();

      // Reload request list & cases queue in background
      const reqListRes = await fetch('http://localhost:8000/api/prior-auth');
      if (reqListRes.ok) {
        const reqData = await reqListRes.json();
        setRequests(reqData);
      }

      // Auto select and bind evaluations/decisions
      setSelectedRequestId(data.authorization_request.request_id);
      setIntakeData({
        clinical_evidence_packet: data.clinical_evidence_packet,
        policy_routing: data.policy_routing,
        policy_retrieval: data.policy_retrieval,
        warnings: data.evaluation_bundle.warnings || []
      });
      setEvaluationData(data.evaluation_bundle);
      setDecisionData(data.decision_support);
          ...
          {showIntakeWorkflow ? (
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                <h2 style={{ fontSize: '18px', margin: 0, color: '#a78bfa' }}>📂 Document-Based Patient Intake</h2>
                <button
                  className="btn-primary"
                  style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid var(--border-color)', padding: '6px 14px', width: 'auto' }}
                  onClick={() => setShowIntakeWorkflow(false)}
                >
                  ← Back to Requests
                </button>
              </div>
              ...
              {!uploadedDocMetadata && (
                <div style={{
                  ...
                  <h3 style={{ fontSize: '15px', marginBottom: '12px' }}>Upload Patient Clinical Document</h3>
                  <p style={{ color: 'var(--text-secondary)', fontSize: '13px', margin: '0 0 20px 0' }}>
                    Supported formats: Native or Scanned PDF, DOCX, TXT (Max size 10MB)
                  </p>

                  <form onSubmit={handleDocumentUploadAndExtract} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
                    <input
                      type="file"
                    ...
                    >
                      {uploadFile ? `Selected: ${uploadFile.name}` : '📁 Choose Clinical Document'}
                    </label>
                    ...
                    {uploadFile && (
                      ...
                      >
                        {uploading ? 'Extracting clinical facts...' : '🚀 Upload & Extract Clinical Facts'}
                      </button>
                    )}
                  </form>
                  ...
                  {uploading && (
                    <div style={{ marginTop: '20px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px' }}>
                      <span className="spinner" style={{ borderColor: '#8b5cf6', borderTopColor: 'transparent', width: '20px', height: '20px' }}></span>
                      <p style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>
                        Parsing text layout structure and running LLM clinical extraction layers...
                      </p>
                    </div>
                  )}
              ...
              {/* Extraction fact review & confirmation panel */}
              {uploadedDocMetadata && extractedFacts && (
                <div>
                  ...
                  <div style={{
                    ...
                    <div><strong>Document ID:</strong> <code>{uploadedDocMetadata.document_id}</code></div>
                    <div><strong>Filename:</strong> <span>{uploadedDocMetadata.filename}</span></div>
                    <div><strong>File Type:</strong> <span style={{ textTransform: 'uppercase' }}>{uploadedDocMetadata.file_type}</span></div>
                    <div><strong>Extraction Status:</strong> <span style={{ color: extractedFacts.status === 'CONFIRMED' ? '#34D399' : '#FBBF24', fontWeight: 'bold' }}>{extractedFacts.status}</span></div>
                    <div><strong>Page Count:</strong> <span>{uploadedDocMetadata.page_count}</span></div>
                    <div><strong>Version:</strong> <code>v{extractedFacts.version || 1}</code></div>
                  </div>

                  {/* Scanned/OCR warning */}
                  {uploadedDocMetadata.warnings && uploadedDocMetadata.warnings.includes('OCR_REQUIRED') && (
                    <div style={{
                      ...
                      ⚠️ Scanned document layout detected (low native character density). Triggered fallback optical text parser simulation.
                    </div>
                  )}

                  {/* Active Provenance highlight card */}
                  {activeProvenanceHighlight && (
                    <div style={{
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px', fontWeight: 'bold', color: '#a78bfa' }}>
                        <span>📍 Source Snippet Provenance ({activeProvenanceHighlight.fact_type})</span>
                        <button
                          style={{ background: 'none', border: 'none', color: '#fff', cursor: 'pointer', fontSize: '12px' }}
                          onClick={() => setActiveProvenanceHighlight(null)}
                        >
                          ✕ Close
                        </button>
                      </div>
                      <p style={{ margin: '0 0 4px 0', fontStyle: 'italic', color: '#e9d5ff' }}>
                        "{activeProvenanceHighlight.source_text}"
                      </p>
                      <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.4)' }}>
                        Document reference page: {activeProvenanceHighlight.page_number} | Extracted via {activeProvenanceHighlight.extraction_method}
                      </div>
                    </div>
                  )}

                  {/* Fact editor fields */}
                  <h3 style={{ fontSize: '15px', marginBottom: '12px', color: '#a78bfa' }}>📝 Review & Correct Extracted Facts</h3>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>

                    {/* Patient & Service Grid */}
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px' }}>
                      <div className="form-group">
                        <label className="form-label" style={{ fontSize: '12px' }}>Patient Name</label>
                        <input
                          type="text"
                          className="form-control"
                          disabled={extractedFacts.status === 'CONFIRMED'}
                          value={extractedFacts.patient?.name || ''}
                          onChange={(e) => {
                            const val = e.target.value;
                            setExtractedFacts((prev: any) => ({
                              ...prev,
                              patient: { ...prev.patient, name: val }
                            }));
                          }}
                        />
                      </div>
                      <div className="form-group">
                        <label className="form-label" style={{ fontSize: '12px' }}>Date of Birth</label>
                        <input
                          type="text"
                          className="form-control"
                          placeholder="YYYY-MM-DD"
                          disabled={extractedFacts.status === 'CONFIRMED'}
                          value={extractedFacts.patient?.dob || ''}
                          onChange={(e) => {
                            const val = e.target.value;
                            setExtractedFacts((prev: any) => ({
                              ...prev,
                              patient: { ...prev.patient, dob: val }
                            }));
                          }}
                        />
                      </div>
                      <div className="form-group">
                        ...
                        <input
                          ...
                          className="form-control"
                          disabled={extractedFacts.status === 'CONFIRMED'}
                          value={extractedFacts.patient?.gender || ''}
                          onChange={(e) => {
                            const val = e.target.value;
                            setExtractedFacts((prev: any) => ({
                              ...prev,
                              patient: { ...prev.patient, gender: val }
                            }));
                          }}
                        />
                      </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px' }}>
                      <div className="form-group">
                        <label className="form-label" style={{ fontSize: '12px' }}>Requested HCPCS/CPT</label>
                        <div style={{ display: 'flex', gap: '6px' }}>
                          <input
                            type="text"
                            className="form-control"
                            disabled={extractedFacts.status === 'CONFIRMED'}
                            value={extractedFacts.requested_service?.code || ''}
                            onChange={(e) => {
                              const val = e.target.value;
                              setExtractedFacts((prev: any) => ({
                                ...prev,
                                requested_service: { ...prev.requested_service, code: val }
                              }));
                            }}
                          />
                          <button
                            type="button"
                            style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid var(--border-color)', borderRadius: '4px', padding: '0 8px', fontSize: '11px', cursor: 'pointer' }}
                            onClick={() => {
                              const prov = extractedFacts.provenance_records?.find((p: any) => p.fact_type === 'requested_procedure_code');
                              if (prov) setActiveProvenanceHighlight(prov);
                              else alert('No specific provenance snippet found for CPT.');
                            }}
                          >
                            📍 Source
                          </button>
                      ...
                      <div className="form-group">
                        <label className="form-label" style={{ fontSize: '12px' }}>Geography State</label>
                        <input
                          type="text"
                          className="form-control"
                          disabled={extractedFacts.status === 'CONFIRMED'}
                          value={extractedFacts.geography?.state || ''}
                          onChange={(e) => {
                            const val = e.target.value;
                            setExtractedFacts((prev: any) => ({
                              ...prev,
                              geography: { ...prev.geography, state: val }
                            }));
                          }}
                        />
                      </div>
                      <div className="form-group">
                        <label className="form-label" style={{ fontSize: '12px' }}>Urgency</label>
                        <input
                          type="text"
                          className="form-control"
                          disabled={extractedFacts.status === 'CONFIRMED'}
                          value={extractedFacts.requested_service?.urgency || 'Standard'}
                          onChange={(e) => {
                            const val = e.target.value;
                            setExtractedFacts((prev: any) => ({
                              ...prev,
                              requested_service: { ...prev.requested_service, urgency: val }
                            }));
                          }}
                        />
                      </div>
                    ...
                    <div style={{ background: 'rgba(255,255,255,0.01)', border: '1px solid var(--border-color)', padding: '16px', borderRadius: '8px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                        <h4 style={{ margin: 0, fontSize: '13px', color: '#a78bfa' }}>ICD-10 Diagnoses</h4>
                        {extractedFacts.status !== 'CONFIRMED' && (
                          <button
                            ...
                            onClick={() => {
                              setExtractedFacts((prev: any) => ({
                                ...prev,
                                diagnoses: [...(prev.diagnoses || []), { code: '', description: '', code_status: 'DOCUMENTED' }]
                              }));
                            }}
                          >
                            + Add Diagnosis
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        {extractedFacts.diagnoses?.map((diag: any, i: number) => (
                          <div key={i} style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                            <input
                              ...
                              onChange={(e) => {
                                const val = e.target.value;
                                const list = [...extractedFacts.diagnoses];
                                list[i] = { ...list[i], code: val };
                                setExtractedFacts((prev: any) => ({ ...prev, diagnoses: list }));
                              }}
                            />
                            <input
                              type="text"
                            ...
                            <button
                              type="button"
                              style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid var(--border-color)', borderRadius: '4px', padding: '8px', cursor: 'pointer' }}
                              onClick={() => {
                                const prov = extractedFacts.provenance_records?.find((p: any) => p.fact_type === 'diagnosis_code' || p.value === diag.code);
                                if (prov) setActiveProvenanceHighlight(prov);
                                else alert('No source snippet provenance mapped for this code.');
                              }}
                            >
                              📍
                            </button>
                            {extractedFacts.status !== 'CONFIRMED' && (
                              <button
                                type="button"
                                style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', color: '#F87171', borderRadius: '4px', padding: '8px', cursor: 'pointer' }}
                                onClick={() => {
                                  const list = extractedFacts.diagnoses.filter((_: any, idx: number) => idx !== i);
                                  setExtractedFacts((prev: any) => ({ ...prev, diagnoses: list }));
                                }}
                              >
                                ✕
                    ...
                    {/* Prior Treatments Array */}
                    <div style={{ background: 'rgba(255,255,255,0.01)', border: '1px solid var(--border-color)', padding: '16px', borderRadius: '8px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                        <h4 style={{ margin: 0, fontSize: '13px', color: '#a78bfa' }}>Prior Treatments & Medication Histories</h4>
                        {extractedFacts.status !== 'CONFIRMED' && (
                          <button
                            type="button"
                            className="btn-primary"
                            ...
                            onClick={() => {
                              setExtractedFacts((prev: any) => ({
                                ...prev,
                                prior_treatments: [...(prev.prior_treatments || []), { treatment_type: 'medication', name: '', duration: '', failed: true }]
                              }));
                            }}
                          >
                            + Add Treatment
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                        {extractedFacts.prior_treatments?.map((pt: any, i: number) => (
                          <div key={i} style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                            <select
                              className="form-control"
                              style={{ width: '130px' }}
                              disabled={extractedFacts.status === 'CONFIRMED'}
                              value={pt.treatment_type || 'medication'}
                              onChange={(e) => {
                                const val = e.target.value;
                                const list = [...extractedFacts.prior_treatments];
                                list[i] = { ...list[i], treatment_type: val };
                                setExtractedFacts((prev: any) => ({ ...prev, prior_treatments: list }));
                              }}
                            >
                            ...
                            <input
                              ...
                              disabled={extractedFacts.status === 'CONFIRMED'}
                              value={pt.name || pt.treatment_name || ''}
                              onChange={(e) => {
                                const val = e.target.value;
                                const list = [...extractedFacts.prior_treatments];
                            ...
                            <button
                              ...
                              style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid var(--border-color)', borderRadius: '4px', padding: '8px', cursor: 'pointer' }}
                              onClick={() => {
                                const prov = extractedFacts.provenance_records?.find((p: any) => p.fact_type === 'prior_treatments' || p.value?.includes(pt.name));
                                if (prov) setActiveProvenanceHighlight(prov);
                                else alert('No source snippet provenance mapped for this treatment.');
                              }}
                            >
                              📍
                            </button>
                            {extractedFacts.status !== 'CONFIRMED' && (
                              <button
                                ...
                                style={{ background: 'rgba(239,68,68,0.1)', border: '1px solid rgba(239,68,68,0.2)', color: '#F87171', borderRadius: '4px', padding: '8px', cursor: 'pointer' }}
                                onClick={() => {
                                  const list = extractedFacts.prior_treatments.filter((_: any, idx: number) => idx !== i);
                                  setExtractedFacts((prev: any) => ({ ...prev, prior_treatments: list }));
                                }}
                              >
                                ✕
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                      <div className="form-group">
                        <label className="form-label" style={{ fontSize: '12px' }}>Clinical Indication / Narrative</label>
                        <textarea
                          className="form-control"
                          rows={3}
                          disabled={extractedFacts.status === 'CONFIRMED'}
                          value={extractedFacts.clinical_indication || ''}
                          onChange={(e) => {
                            const val = e.target.value;
                            setExtractedFacts((prev: any) => ({ ...prev, clinical_indication: val }));
                          }}
                        />
                      </div>
                      <div className="form-group">
                        <label className="form-label" style={{ fontSize: '12px' }}>Provider Justification / Referral</label>
                        <textarea
                          className="form-control"
                          rows={3}
                          disabled={extractedFacts.status === 'CONFIRMED'}
                          value={extractedFacts.provider_justification || ''}
                          onChange={(e) => {
                            const val = e.target.value;
                            setExtractedFacts((prev: any) => ({ ...prev, provider_justification: val }));
                          }}
                        />
                      </div>
                    </div>
                    ...
                    {extractedFacts.edit_history && extractedFacts.edit_history.length > 0 && (
                      <div style={{ background: 'rgba(255,255,255,0.01)', border: '1px solid var(--border-color)', padding: '16px', borderRadius: '8px' }}>
                        <h4 style={{ margin: '0 0 8px 0', fontSize: '13px', color: 'var(--text-secondary)' }}>✏ Manual Correction History</h4>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', fontSize: '12px' }}>
                          {extractedFacts.edit_history.map((hist: any, idx: number) => (
                            <div key={idx} style={{ padding: '6px', borderLeft: '2px solid #8b5cf6', paddingLeft: '10px' }}>
                              <div>
                                <strong>Reviewer:</strong> <code>{hist.reviewer_id}</code> | <span>{new Date(hist.timestamp).toLocaleString()}</span>
                              </div>
                              <div style={{ marginTop: '2px', color: 'rgba(255,255,255,0.6)' }}>
                                Modified fields: Original state values replaced with reviewer corrections.
                              </div>
                            </div>
                          ))}
                        </div>
                      ...
                      {extractedFacts.status !== 'CONFIRMED' ? (
                        <>
                          <button
                            type="button"
                            className="btn-primary"
                            style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid var(--border-color)', color: '#fff', width: 'auto' }}
                            disabled={confirming}
                            onClick={handleSaveDraftCorrections}
                          >
                          ...
                          <button
                            type="button"
                            className="btn-primary"
                            style={{ background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)', width: 'auto' }}
                            disabled={confirming}
                        ...
                        <div style={{ width: '100%' }}>
                          <div style={{
                            ...
                            <h4 style={{ margin: '0 0 12px 0', fontSize: '13px', color: '#10b981' }}>✓ Facts Confirmed & Locked</h4>

                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '16px', marginBottom: '12px' }}>
                              <div className="form-group">
                                <label className="form-label" style={{ fontSize: '12px' }}>CPT/HCPCS Code</label>
                                <input
                                  type="text"
                                  className="form-control"
                                  value={intakeHcpcsOverride}
                                  onChange={(e) => setIntakeHcpcsOverride(e.target.value)}
                                  placeholder="e.g. 97110"
                                />
                              </div>
                              <div className="form-group">
                                <label className="form-label" style={{ fontSize: '12px' }}>State Code Override</label>
                                <input
                                  type="text"
                                  className="form-control"
                                  value={intakeStateOverride}
                                  onChange={(e) => setIntakeStateOverride(e.target.value)}
                                  placeholder="e.g. CO"
                                />
                              </div>
                              <div className="form-group">
                                <label className="form-label" style={{ fontSize: '12px' }}>Date of Service Override</label>
                                <input
                                  type="date"
                                  className="form-control"
                                  value={intakeDosOverride}
                                  onChange={(e) => setIntakeDosOverride(e.target.value)}
                                />
                              </div>
                            </div>
