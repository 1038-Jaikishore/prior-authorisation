...
class PriorAuthorizationIntakeService:
    ...
    def compile_evidence_packet(
        ...
        if not state_code or state_code == "CO":
            state_code = "Colorado"
        elif state_code == "UNKNOWN":
            state_code = "UNKNOWN"
        ...
        packet = ClinicalEvidencePacket(
            ...
            icd_10_pcs_codes=req_doc.get("icd_10_pcs_codes", []),
            modifiers=req_doc.get("modifiers", []),
            revenue_codes=req_doc.get("revenue_codes", []),
    ...
    def execute_route_and_retrieve(
        ...
        if not scope_has_records:
            ...
            retrieval_res = PolicyRetrievalService.retrieve_policy_chunks(
                query=f"Coverage requirements, indications, and limitations for service {hcpcs_code}. Patient clinical conditions: {clinical_context}",
