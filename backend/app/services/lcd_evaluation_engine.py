...
class LCDEvaluationEngine:
    ...
    def evaluate_lcds(
        ...
        """
        Evaluates clinical evidence against retrieved LCD chunks to determine coverage.
        """
        openrouter_key = os.environ.get("OPENROUTER_API_KEY")
        if not openrouter_key:
            raise ValueError("OPENROUTER_API_KEY is not set in environment.")

        # 1. Filter Retrieval Results for LCDs only
        lcd_chunks = [
            chunk for chunk in retrieval_result 
            if chunk.get("document_type") in ["LCD", "Article"]
        ]

        if not lcd_chunks:
            return LCDSemanticEvaluationResult(
                lcd_determination="NOT ADDRESSED",
                semantic_similarity_score=0.0,
                confidence_score=0.0,
                key_policy_excerpts=[],
                conditions=[],
                reasoning="No LCD text paragraphs were retrieved for this request."
            )

        # 2. Build the LLM Context
        # Format Clinical Facts
        facts = {
            "requested_service": clinical_evidence.requested_service.get("description"),
            "diagnosis_codes": clinical_evidence.diagnosis_codes,
            "conditions": [c.get("name") for c in clinical_evidence.conditions] if clinical_evidence.conditions else [],
            "clinical_assessments": [a.get("name") for a in clinical_evidence.clinical_assessments] if (hasattr(clinical_evidence, 'clinical_assessments') and clinical_evidence.clinical_assessments) else [],
            "diagnostic_results": clinical_evidence.diagnostic_results if (hasattr(clinical_evidence, 'diagnostic_results') and clinical_evidence.diagnostic_results) else [],
            "encounters": [f"{e.get('type')} with {e.get('provider_specialty')}" for e in clinical_evidence.encounters] if (hasattr(clinical_evidence, 'encounters') and clinical_evidence.encounters) else [],
            "age": clinical_evidence.demographics.get("age"),
            "gender": clinical_evidence.demographics.get("gender")
        }
        ...
        for chunk in lcd_chunks:
            ...
            text = chunk.get("text")
            law_texts.append(f"[LCD {doc_id} | Section: {section}]\n{text}")
            
        law_context = "\n\n".join(law_texts)

        # 3. Construct Prompt
        system_prompt = """You are a highly precise Medicare Prior Authorization AI Agent. 
Your task is to evaluate the provided Patient Clinical Facts against the provided Medicare LCD (Local Coverage Determination) Law Text.

You must output a raw JSON object (without markdown code blocks) matching exactly this structure:
{
  "lcd_determination": "COVERED" | "NOT COVERED" | "NOT ADDRESSED",
  "semantic_similarity_score": 0.0,
  "confidence_score": 0.0,
...
"""
        ...
        try:
            ...
            if parsed_data.get('lcd_determination') != 'NOT ADDRESSED' and parsed_data.get('requirements_evaluated'):
                ...
                if global_pathway and global_pathway['status'] == 'FAILED':
                    parsed_data['lcd_determination'] = 'NOT COVERED'
                elif global_pathway and global_pathway['status'] in ['MISSING', 'AMBIGUOUS']:
                    parsed_data['lcd_determination'] = 'NOT ADDRESSED'
                else:
                    # Global is MET or non-existent. Evaluate alternatives.
                    if not alt_pathways:
                        parsed_data['lcd_determination'] = 'COVERED'
                        ...
                        parsed_data['lcd_determination'] = 'NOT ADDRESSED' # Will map to NURSE_REVIEW
                    else:
                        parsed_data['lcd_determination'] = 'NOT ADDRESSED' # Will map to PEND
            else:
                parsed_data['pathways_evaluated'] = []
            
            return LCDSemanticEvaluationResult(**parsed_data)
